---
type: source
source_type: paper
title: "The Effects of MARIO, a Social Robot, on the Resilience of People with Dementia: A Multiple Case Study"
apa7: "Whelan, S., Burke, M., Barrett, E., Mannion, A., Kovacic, T., Santorelli, A., ... Casey, D. (2020). The effects of MARIO, a social robot, on the resilience of people with dementia: A multiple case study. Gerontechnology, 19(4), 7-18. https://doi.org/10.4017/gt.2020.19.issue-4"
doi: https://doi.org/10.4017/gt.2020.20.007.09
appearances:
  - unit: FXA303
    week: "13"
tags: ["case-study-research", "dementia-care", "facilitated-sessions", "fxa303", "resilience-building", "social-robot-interaction", "social-robotics", "technology-assisted-interventions"]
key_concepts: ["resilience", "social-robot-interaction", "facilitated-sessions"]
methods: ["multiple-case-study"]
slug: fxa303-week13-the-effects-of-mario-a-social-robot-on-the-resilience-of
related: ["case-study-research", "dementia-care", "resilience-building"]
created: 2026-05-15
updated: 2026-05-15
---

_All claims sourced from ../raw/pdf/fxa303-week13-the-effects-of-mario-a-social-robot-on-the-resilience-of.md_

## The Effects of MARIO, a Social Robot, on the Resilience of People with Dementia: A Multiple Case Study

This study investigates the impact of MARIO, a social robot designed to enhance social connectedness among people with dementia, on their resilience. The research utilized multiple case study methodology involving twelve facilitated sessions where participants interacted with MARIO. Data were collected through various methods including dementia care mapping, semi-structured interviews, and questionnaires. Analysis revealed that while the robot positively influenced resilience in eight out of ten participants during interactions, skilled facilitation was necessary for effective use. The findings suggest that social robots can support resilience but require enhanced capabilities to interpret emotional needs autonomously. Key themes include minute-to-minute resilience changes, initiating readiness, co-creation of meaningful activities, and overall impact on resilience.

## Key Concepts

- [resilience](../topics/resilience.md)

## Tags

- [social-robotics](../tags/social-robotics.md)
- [dementia-care](../tags/dementia-care.md)
- [resilience-building](../tags/resilience-building.md)
- [case-study-research](../tags/case-study-research.md)
- [technology-assisted-interventions](../tags/technology-assisted-interventions.md)
- [fxa303](../tags/fxa303.md)

## Methods

- **multiple-case-study**

## See Also

- [case-study-research](../topics/case-study-research.md)
- [dementia-care](../topics/dementia-care.md)
- [resilience-building](../topics/resilience-building.md)